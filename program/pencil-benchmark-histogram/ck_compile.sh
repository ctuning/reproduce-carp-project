...
echo "    .$CK_CXX -x c -c $CK_PROG_COMPILER_FLAGS $CK_PROG_COMPILER_VARS -DNDEBUG -fomit-frame-pointer -fPIC -std=c99 $HEADER_FLAGS -I$BENCHMARK_ROOT_DIRECTORY/$KERNEL ${KERNEL}.pencil_host.c -o $KERNEL.pencil_host.o" 
$CK_CXX -x c -c $CK_PROG_COMPILER_FLAGS $CK_PROG_COMPILER_VARS -DNDEBUG -fomit-frame-pointer -fPIC -std=c99 $HEADER_FLAGS -I$BENCHMARK_ROOT_DIRECTORY/$KERNEL ${KERNEL}.pencil_host.c -o $KERNEL.pencil_host.o 
EXIT_STATUS_COMPILATION_1=$?

echo "    .$CK_CXX -shared $CK_PROG_COMPILER_FLAGS $CK_PROG_COMPILER_VARS -o lib${KERNEL}_ppcg.so $KERNEL.pencil_host.o $LINKER_FLAGS $LIBRARY_FLAGS" 
$CK_CXX -shared  $CK_PROG_COMPILER_FLAGS $CK_PROG_COMPILER_VARS -o lib${KERNEL}_ppcg.so $KERNEL.pencil_host.o $LINKER_FLAGS $LIBRARY_FLAGS 
EXIT_STATUS_COMPILATION_2=$?

echo "    .$CK_CXX  $CK_PROG_COMPILER_FLAGS $CK_PROG_COMPILER_VARS $DEFINED_VARIABLES -fomit-frame-pointer -fPIC -std=c++0x $HEADER_FLAGS -Wl,-rpath=RIGIN:$PRL_LIB_DIR $BENCHMARK_ROOT_DIRECTORY/$KERNEL/test_${KERNEL}.cpp -o $CK_PROG_TARGET_EXE $LIBRARY_FLAGS $LINKER_FLAGS -l${KERNEL}_ppcg" 
$CK_CXX  $CK_PROG_COMPILER_FLAGS $CK_PROG_COMPILER_VARS $DEFINED_VARIABLES -fomit-frame-pointer -fPIC -std=c++0x $HEADER_FLAGS -Wl,-rpath=RIGIN:$PRL_LIB_DIR $BENCHMARK_ROOT_DIRECTORY/$KERNEL/test_${KERNEL}.cpp -o $CK_PROG_TARGET_EXE $LIBRARY_FLAGS $LINKER_FLAGS -l${KERNEL}_ppcg 
