...
echo "Current TBB LIB DIR: ${CK_ENV_LIB_TBB_LIB}"

cmake ../${PACKAGE_NAME} -DWITH_GSTREAMER=OFF -DWITH_CUDA=OFF -DWITH_OPENCL=ON ${CK_PARAM_CMAKE_CONFIG} -DWITH_TBB=ON -DTBB_INCLUDE_DIR=${CK_ENV_LIB_TBB_INCLUDE} -DTBB_LIB_DIR=${CK_ENV_LIB_TBB_LIB} -DCMAKE_INSTALL_PREFIX:PATH=$PWD/install
